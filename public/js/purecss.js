// Pure CSS Admin Portal JavaScript
    ...
    row.innerHTML = `
        <td>
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=0078e7&color=fff" 
        ...
        <td>
            <button class="pure-button small" onclick="editUser(${user.id})" style="background: #f0f0f0; color: #333; margin-right: 0.5rem;">
                Edit
            </button>
            <button class="pure-button small" onclick="deleteUser(${user.id})" style="background: #dc3545; color: white;">
                Delete
            </button>
        </td>
    `;
...
function updatePageContent(tab) {
    ...
    const subtitles = {
        dashboard: 'Welcome to your Pure CSS admin dashboard',
    ...
    document.getElementById('pageSubtitle').textContent = subtitles[tab] || 'Welcome to your Pure CSS admin dashboard';
...
function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
        ...
        top: 100px;
        right: 20px;
        background: ${type === 'success' ? '#0078e7' : type === 'error' ? '#dc3545' : '#6c757d'};
        color: white;
        padding: 1rem 2rem;
        border-radius: 4px;
