// Foundation Admin Portal JavaScript
...
// Initialize Foundation and the application
    ...
    row.innerHTML = `
        <td>
            <div class="grid-x align-middle">
                <div class="cell shrink">
                    <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=1779ba&color=fff" 
                         alt="Avatar" class="user-avatar" style="width: 32px; height: 32px; border-radius: 50%; margin-right: 0.5rem;">
                </div>
                <div class="cell auto">
                    <strong>${user.name}</strong><br>
                    <small class="subheader">${user.email}</small>
                </div>
            </div>
        </td>
        <td>${user.email}</td>
        ...
        <td>
            <div class="button-group">
                <button class="button small secondary" onclick="editUser(${user.id})">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="button small alert" onclick="deleteUser(${user.id})">
...
function setActiveTab(tab) {
    ...
    document.querySelectorAll('.sidebar .vertical.menu > li').forEach(li => {
...
function updatePageContent(tab) {
    ...
    const subtitles = {
        dashboard: 'Welcome to your Foundation admin dashboard',
    ...
    document.getElementById('pageSubtitle').textContent = subtitles[tab] || 'Welcome to your Foundation admin dashboard';
...
function deleteUser(userId) {
    if (confirm('Are you sure you want to delete this user?')) {
        ...
        if (index > -1) {
            ...
            refreshUserTable();
            showNotification('User deleted successfully!', 'alert');
        }
    }
}
...
// Show notification using Foundation callout
function showNotification(message, type = 'primary') {
    const notification = document.createElement('div');
    notification.className = `callout ${type} notification`;
